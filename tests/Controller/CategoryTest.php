...
class CategoryTest extends BaseWeb
{
    ...
    public function testCategory()
    {
        ...
        $languages = self::$container->get('doctrine')
            ->getRepository(Language::class)
            ->findBy(['status' => '1']);
        foreach ($languages as $language) {
            $categories = self::$container
                ->get('doctrine')
                ->getRepository(Category::class)
                ->findBy(['language' => [$language->getId(), null]]);
                    ...
                    for ($page = 1; $page <= $number_page; $page++) {
                        if ($language->getIsoCode()=='en') {
                            $crawler = $this->client->request(
                                'GET',
                                '/category/' . $category->getSlug() . '/' . $page
                            );
                        } elseif ($language->getIsoCode()=='pl') {
                            $crawler = $this->client->request(
                                'GET',
                                '/kategoria/' . $category->getSlug() . '/' . $page
                            );
                        }
                        ...
                        foreach ($posts as $post) {
                            if ($language->getIsoCode()=='en') {
                                $this->assertEquals(
                                    1,
                                    $crawler->filter(
                                        '.post-preview a[href="http://localhost/post/' . $post->getSlug() . '"]'
                                    )->count()
                                );
                            } elseif ($language->getIsoCode()=='pl') {
                                $this->assertEquals(
                                    1,
                                    $crawler->filter(
                                        '.post-preview a[href="http://localhost/wpis/' . $post->getSlug() . '"]'
                                    )->count()
                                );
                            }
                            ...
                            $this->assertEquals(
                                1,
                                $crawler->filter('h2.post-title:contains("' . $post->getTitle() . '")')
                                ->count()
                            );
                            $this->assertEquals(
                                1,
                            ...
                            $this->assertEquals(
                                1,
                                $crawler->filter(
                                    'p.post-meta:contains("' . $post->getDate()->format('H:i:s d M Y') . '")'
                                )->count()
                            );
                        }
                        ...
                        if ($page > 1) {
                            if ($language->getIsoCode()=='en') {
                                $this->assertEquals(
                                    1,
                                    $crawler->filter('#newer-posts:contains("Newer Posts")')->count()
                                );
                            } elseif ($language->getIsoCode()=='pl') {
                                $this->assertEquals(
                                    1,
                                    $crawler->filter('#newer-posts:contains("Nowsze Wpisy")')->count()
                                );
                            }
                            $newer_page = $page - 1;
                        ...
                        if ($page < $number_page) {
                            if ($language->getIsoCode()=='en') {
                                $this->assertEquals(
                                    1,
                                    $crawler->filter('#older-posts:contains("Older Posts")')->count()
                                );
                            } elseif ($language->getIsoCode()=='pl') {
                                $this->assertEquals(
                                    1,
                                    $crawler->filter('#older-posts:contains("Starsze Wpisy")')->count()
                                );
                            }
                            $older_page = $page + 1;
