    ...
    public function testPost()
    {
        ...
        foreach ($languages as $language) {
            if ($language->getIsoCode() == 'en') {
                ...
                $crawler = $this->client->request('GET', '/wpis');
                $this->assertEquals(
                    1,
                    $crawler->filter('h1:contains("Przepraszamy ale nia ma wpisu pod tym adresem")')
                        ->count()
                );
            }
            ...
            foreach ($random_posts as $post) {
                ...
                $this->assertEquals(
                    1,
                    $crawler->filter('h2.subheading:contains("' . $post->getSubTitle() . '")')->count()
                );
                $this->assertEquals(
                    1,
                    $crawler->filter('span.meta:contains("' . $post->getDate()->format('H:i:s d M Y') . '")')
                        ->count()
                );
