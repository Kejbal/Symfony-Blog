...
class CategoryAdminTest extends BaseWeb
{
    ...
    public function testAddEdit()
    {
        ...
        $category = self::$container
            ...
            ->findOneBy(array('name' => 'Tessfdt', 'language' => '1','slug'=>'tessfdt'));
        ...
        $category = self::$container
            ->get('doctrine')
            ->getRepository(Category::class)
            ->findOneBy(array('name' => 'Tessfdt', 'language' => '1', 'slug'=>'tessfdt-0'));
        $this->assertTrue(!empty($category));
        $this->assertTrue($crawler->filter('html:contains("Redirecting to")')->count() > 0);
        ...
        $category = self::$container
            ->get('doctrine')
            ->getRepository(Category::class)
            ->findOneBy(array('name' => 'Hu4thahr', 'language' => '2', 'slug'=>'hu4thahr'));
        $this->assertTrue(!empty($category));
        ...
        $crawler = $this->client->submit($form);
        $category = self::$container->get('doctrine')
            ->getRepository(Category::class)
            ->findOneBy(array('name' => 'Hu4thahr', 'language' => '2','slug'=>'hu4thahr'));
        $this->assertTrue(!empty($category));
